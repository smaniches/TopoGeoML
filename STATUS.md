...
4. **The corrected sheaf replication H009-R shows the learned scalar sheaf construction behaving like the fixed Hodge operator, not improving on it.** The historical H009 implementation was invalidated (both edge orientations processed independently, so the learned matrix was not guaranteed to be the claimed `delta.T @ delta` cellular-sheaf Laplacian; 2,468 parameters versus 2,338, outside the 5% tolerance) and its numbers remain provenance only. The repaired `sheaf-residual` 2.0.0 (one coboundary row per undirected edge, symmetric PSD by construction, invariant-tested, 2,403 parameters, +2.78%) was rerun under the preregistered H009-R protocol on NCI1 with 30 seeds. Result: sheaf median 0.604 versus MLP 0.523 (p_BH = 4.74 x 10^-3, H39 supported), versus Hodge 0.605 (p_BH = 0.428, H40 not supported, no equivalence claim), and versus gin-residual 0.630 (p_BH = 0.0342, below gin on 22/30 seeds; H41's preregistered 0.01 falsification threshold is not crossed, so H41 is inconclusive). Within this regime the learned restriction maps add no measurable value over the fixed Hodge operator.
