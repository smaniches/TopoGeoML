...
**Validated (mixed; positive equality).** A one-layer Hodge MP classifier with a *symmetrically-normalised* Laplacian is statistically indistinguishable from an MLP baseline of matched capacity on **MUTAG** (30 seeds, 20 epochs, median Δ = +0.000, paired Wilcoxon p_BH = 0.714) and on **PROTEINS** (30 seeds, 10 epochs, median Δ = +0.0135, p_BH = 0.548). Two-dataset equality claim. On MUTAG the unnormalised combinatorial variant underperforms by 9 pp (p_BH = 5.66 × 10⁻⁴); on PROTEINS the combinatorial-vs-normalised contrast vanishes (Δ = 4.3 pp, p_BH = 0.34) — the MUTAG normalisation effect does *not* generalise to larger graphs. See `notebooks/results/mutag_hodge_ablation_30seeds.md` and `notebooks/results/proteins_hodge_ablation_30seeds.md`; the preregistered hypothesis docs are at `docs/hypotheses/HYPOTHESIS-001-hodge-mutag.md` and `docs/hypotheses/HYPOTHESIS-002-hodge-proteins.md`.

**Refuted (strong positive-difference claim).** No Hodge architecture in the current registry (combinatorial, symm-normalised, normalised + residual, normalised + 2-stacked + residual) **beats** an MLP baseline on either MUTAG or PROTEINS at p_BH < 0.01 after BH correction. The strong "topology helps graph classification" claim is ruled out at this architectural scale on these two datasets. Hypothesis 003 (deferred) tests either an architectural escalation (HL-HGAT attention, polynomial filters, SCConv up-down) or a scale escalation (NCI1, DD, COLLAB).
